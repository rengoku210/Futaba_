...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300, constraint_lines: tuple[str, ...] = (),
    ...
    *constraint_lines* pins the resolver (plugin installs pass Hermes' own declared ranges so a plugin
    ...
    try:
        ...
        candidate_venv = Path(sys.executable).parent.parent
        if (candidate_venv / "pyvenv.cfg").exists():
            uv_env["VIRTUAL_ENV"] = str(candidate_venv)
        elif (Path(sys.executable).parent / "pyvenv.cfg").exists():
            uv_env["VIRTUAL_ENV"] = str(Path(sys.executable).parent)
